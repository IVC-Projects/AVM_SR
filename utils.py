...
def make_coord(shape, ranges=None, flatten=True):
    ...
    return ret
def to_pixel_samples(gt_H,gt_W):
    """ Convert the image to coord-RGB pairs.
        img: Tensor, (3, H, W)
    """
    coord = make_coord([int(gt_H),int(gt_W)])
    return coord
#####
...
def _load_img_array(path, color_mode='RGB', channel_mean=None, modcrop=[0 ,0 ,0 ,0] ,norm=True):
...
def _getYdata(path, size,norm=True):
    w= int(size[0])
    h= int(size[1])
    #print(w,h)
...
def _getHW(path,idx=-1):
    res = path.split(os.sep)[-1]
    res = res.split(".")[-2]
    res = res.split("_")[idx]
    return res.split("x")
...
class DirectoryIterator_DIV2K(Iterator):
    def __init__(self,
        ...
        import glob
        path = os.path.join(datadir, 'DIV2K_train_LR_AVM', "2by1", 'qp{}'.format(qp), '*.yuv')
        lrs = glob.glob(path)
        # lrs = glob.glob(datadir + '/DIV2K_train_LR_bicubic/X{}/*.png'.format(scale_factor))
        # lrs.sort()

        path = os.path.join(datadir, 'DIV2K_train_HR', 'label', '*.yuv')
        sharps = glob.glob(path)
        ...
        super(DirectoryIterator_DIV2K, self).__init__(self.total_count, out_batch_size // crop_per_image, shuffle, seed,
                                                      infinite)
    ...
    def next(self):
        ...
        while (len(batch_blur) < self.out_batch_size):
            ...
            try:
                ...
                W, H = _getHW(sharps)
                B_ = _getYdata(blurs, [int(W) / self.r, int(H) / self.r], norm=True)
                S_ = _getYdata(sharps, [W, H], norm=True)
                B_ = B_[:, :, None]
                S_ = S_[:, :, None]
            except:
...
class DirectoryIterator_DIV2K_3by2(Iterator):
    def __init__(self,
                 datadir ='.train/',
                 qp =255,
                 crop_size =32,
        ...
        import glob
        path = os.path.join(datadir, 'DIV2K_train_LR_AVM', "{}by{}".format(str(up_factor),str(dwon_factor)), 'qp{}'.format(qp), '*.yuv')
        lrs = glob.glob(path)
        # lrs = glob.glob(datadir + '/DIV2K_train_LR_bicubic/X{}/*.png'.format(scale_factor))
        # lrs.sort()
        ...
        super(DirectoryIterator_DIV2K_3by2, self).__init__(self.total_count, out_batch_size // crop_per_image, shuffle, seed,
                                                      infinite)
    ...
    def next(self):
        ...
        while (len(batch_blur) < self.out_batch_size):
            ...
            try:
                ...
                # S_ = _load_img_array(sharps,color_mode="Y")
                W, H = _getHW(blurs)
                B_ = _getYdata(blurs, [W, H], norm=True)
                W, H = _getHW(sharps)
                S_ = _getYdata(sharps, [W, H], norm=True)
                B_ = B_[:, :, None]
                S_ = S_[:, :, None]
            ...
            for j in range(self.crop_per_image):
                ...
                r=self.up/self.down
                B = B_[sh:sh + self.crop_size, sw:sw + self.crop_size]
                S = S_[round(sh * r):round((sh + self.crop_size) * r), round(sw * r):round((sw + self.crop_size) * r)]
